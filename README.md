...
```bash
dk bash  # starts bash inside "url_shortener" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and url_shortener.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `url_shortener` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The url_shortener container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv url_shortener -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database url_shortener owned by a user url_shortener.

On your remote machine, create a file ~/url_shortener.env:

```
DJANGO_DB_PASSWORD=<url_shortener's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=url_shortener
DJANGO_DB_USER=url_shortener
DJANGO_DEBUG=0
```
...
```
server {
    server_name  url_shortener.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/url_shortener/static;
...
(Replace "url_shortener.example.com" with your production domain)
