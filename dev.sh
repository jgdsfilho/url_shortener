...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=url_shortener.example.com
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development url_shortener npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop url_shortener
    docker rm url_shortener
    docker run --name url_shortener -d --env-file /home/ubuntu/url_shortener.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/url_shortener:/dkdata \
        url_shortener start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./url_shortener/
...
echo_green "Welcome to url_shortener's dev env"
